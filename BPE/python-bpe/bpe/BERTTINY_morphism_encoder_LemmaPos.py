...
class Encoder:
    ...
    ### now impelmented for lemma pos change if not lemma pos to original function of this..
    def byte_pair_counts(self, words):
        ...
        for token, count in self._progress_bar(self.count_tokens(words).items()):
            ...
            #print(compact_token)
            #print(compact_token[0:3])
            #print(compact_token[-3:])
            #print(compact_token[0:3] == '**<')
            #print(compact_token[0:3] is '**<' )
            #print(compact_token[-3:] == '>**')
            #print(compact_token[-3:] is '>**' )
            if compact_token[0:3] == '**<' and compact_token[-3:] == '>**':
                #print("yes")
                bp_counts[compact_token] += (int(2**50) + count)
                #print(token)
                print(compact_token)
                print(f"above tokens count is {count}")
                i += 1
                print(f"number of distinct pos-tags: {i}")
                if compact_token is "<vnw><excl><pron><stan><vol><3><getal>":
                    print("TOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO")
                    print(count)
            #                continue
            else:
                #print("no")
    ...
    def fit(self, text):
        ...
        for sentence in _text:
            ...
            for i in range(len(splitted)):
                #if len(splitted[i]) > 6:
                if i < len(splitted):
                    if splitted[i][0:3] == '**<' and splitted[i][-3:] == '>**':
                        tags.append(splitted[i])
                        del splitted[i]
            #print(splitted) 
            all_text.append(" ".join(splitted))
            if number % 100000 == 0:
                all_all_text = all_all_text + all_text
                all_text = []
        ...
        all_all_text = all_all_text + all_tags
        print(tags[:10])
        print(all_text[:3])
        print(all_text[-3:])
       
        print("Third and a half")
        remaining_words = [word for word in toolz.concat(map(self.word_tokenizer_fitting, all_all_text))
    ...
    def transform(self, sentences, reverse=False, fixed_length=None):
        ...
        ID_encoding, ID_encoding_without_padding = next(self._transform(sentences, reverse, fixed_length))
        ...
        if already_has_special_tokens:
            for id in token_ids_0:
                if id == (self.word_vocab[self.PAD] or self.bpe_vocab[self.SOW] or self.bpe_vocab[self.EOW] or
                          self.bpe_vocab[self.cls_token] or self.bpe_vocab[self.sep_token]):
