...
fixed_len = 128
from BERTTINY_encoder_with_TwoHashTags import Encoder
...
europarl_dataset_1000k_2000k = europarl_dataset[1000000:]
...
#    lines = f.readlines()
#
# oscar_dataset = lines[:2000000] #newlines still
# #oscar_dataset = oscar_dataset_newlines
# oscar_dataset_1000k_2000k = oscar_dataset[1000000:2000000]
...
### BPE HASTAGS                 Ucto..Ucto
#encoder.fit(europarl_dataset[:1500000])
#
#creating_and_fitting_time = format_time(time.time() - creating_and_fitting_time_start)
#print(f"creating_and_fitting_time is: {creating_and_fitting_time}")
#print("finished fit")

#encoder.save("SavedTokenizers/BPEHash1500k.json")
#print("finished save")


#encoder = Encoder.load('SavedTokenizers/FrogMorph1000k128seq.json')
#encoder = Encoder.load('SavedTokenizers/FrogMorph1500k.json')
encoder = Encoder.load('SavedTokenizers/BPEHash1500k.json')
...
listOfVocab = encoder.bpe_vocab.keys()

number =0
second = 0
for i in listOfVocab:
    if i[0:2] == "##":
        number+=1
    else:
        second+=1
...
print(f"##: {number}")
print(f"normal: {second}")

random = "dit is een random zin om het segmentatie process te bepalen en te bekijken van hashtags gastvrije gastronomie"
id= encoder.transform(random)
text = encoder.inverse_transform_list(id)
print(random)
print(text)
...
i = 0
europarl_length = len(europarl_dataset_1000k_2000k)
europarl_sentences_ids = []
for sentence in europarl_dataset_1000k_2000k:
...
#
# i = 0
# oscar_length = len(oscar_dataset_1000k_2000k)
# oscar_sentences_ids = []
# for sentence in oscar_dataset_1000k_2000k:
#     sentence_ids,sentence_ids_without_padding = encoder.transform(sentence.rstrip('\n'), fixed_length=fixed_len)
#     if i % 1000 == 0:
...
# pd.Series(sentences_ids_len).hist()
# pd.Series(sentences_ids_len).describe().to_csv("InfoDatasetTINYBERT/oscar_1000k_2000k_BPEHash.csv")
# print(pd.Series(sentences_ids_len).describe())
# plt.xlabel = "Sentence Length"
# plt.ylabel = "Amount of sentences"
# #plt.legend()
# plt.savefig("InfoDatasetTINYBERT/oscar_1000k_2000k_BPEHash.png",format='png')
...
with open('TrainFiles-BERT-TINY/BPEHash_europarl_sentences_ids_1000k_2000k_128seq.pickle', 'wb') as outputfile:
 pickle.dump(europarl_sentences_ids, outputfile)

#with open('TrainFiles-BERT-TINY/BPEHash_oscar_sentences_ids_1000k_2000k_128seq.pickle', 'wb') as outputfile:
...
sentences_ids_len = [len(x) for x in for_length_europarl_sentences_ids]
pd.Series(sentences_ids_len).hist()
pd.Series(sentences_ids_len).describe().to_csv("InfoDatasetTINYBERT/europarl_1000k_2000k_BPEHash.csv")
print(pd.Series(sentences_ids_len).describe())
plt.xlabel = "Sentence Length"
plt.ylabel = "Amount of sentences"
#plt.legend()
plt.savefig("InfoDatasetTINYBERT/europarl_1000k_2000k_BPEHash.png",format='png')
