...
class Encoder:
    ...
    def __init__(self, vocab_size=8192, pct_bpe=0.2, word_tokenizer=None,
        ...
        self.morph_vocab_size = vocab_size - self.word_vocab_size
    ...
    def learn_morph_vocab(self, words):
        morph_counts = self.morph_counts_fastest_version(words)
        sorted_morph_counts = sorted(morph_counts.items(), key=lambda p: -p[1])
        return {morph: idx for idx, (morph, count) in enumerate(sorted_morph_counts[:self.morph_vocab_size])}
    ...
    def fit(self, text):
        ...
        print("MORPH VOCAB")
        print(self.morph_vocab)
        print("END OF MORPH VOCAB")
    ...
    def _transform(self, sentences, reverse=False, fixed_length=None):
        ...
        for sentence in self._progress_bar(sentences):
            ...
            ###PADDING HIER NOG AAN RECHTERKANT EN HIER NOG NIET GOEIE LENGTE DENK IK, ZIE CLASSIC GOOGLE COLLAB
