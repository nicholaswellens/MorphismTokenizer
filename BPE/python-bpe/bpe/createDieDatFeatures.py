...
fixed_len = 256
from BERTTINY_morphism_encoder_with_merge_for_diedat import Encoder
...
#Dev morph merge time:

with open('TrainFiles-Die-Dat/dev.labels', 'r') as inputfile:
    diedat_dev_labels = inputfile.readlines()

with open('TrainFiles-Die-Dat/dev.sentences', 'r') as inputfile:
    diedat_dev_sentences = inputfile.readlines()

print("ok")
print(diedat_dev_sentences[:2])
print(diedat_dev_labels[:2])

#Tester = change_text_to_morphs(diedat_dev_sentences,  save = True, filename = 'TrainFiles-Die-Dat/FrogMorphedMergedDieDatDev.pickle', frog_merge=True)

with open('TrainFiles-Die-Dat/train.sentences', 'r') as inputfile:
    diedat_train_sentences = inputfile.readlines()

with open('TrainFiles-Die-Dat/train.labels', 'r') as inputfile:
    diedat_train_labels = inputfile.readlines()

print("ok")
print(diedat_train_sentences[:2])
print(diedat_train_labels[:2])

#Tester = change_text_to_morphs(diedat_train_sentences,  save = True, filename = 'TrainFiles-Die-Dat/FrogMorphedMergedDieDatTrain.pickle', frog_merge=True)


with open('TrainFiles-Die-Dat/FrogMorphedMergedDieDatDev.pickle', 'rb') as inputfile:
    FrogMorphedMergedDieDatDev = pickle.load(inputfile)

print("HERE")
print("")
print(FrogMorphedMergedDieDatDev[0])
print("")
print("THERE")



for index in range(len(diedat_train_sentences)):
    label_newline = diedat_train_labels[index]
    sentence_newline = diedat_train_sentences[index]
    sentence = sentence_newline.rstrip('\n')
    label = label_newline.rstrip('\n')
    two_parts = sentence.split("<sep>")
    first_part = two_parts[0]
    second_part = two_parts[1]
    first_part_list = first_part.split(" ")
    second_part_list = second_part.split(" ")
    output_label = 199


diedat_train_sentences_masked = []
diedat_train_labels_masked = []
...
for index in range(len(diedat_train_sentences)):
    ...
    first_part_list = first_part.split(" ")
    second_part_list = second_part.split(" ")
    print(len(first_part_list))
    print(len(second_part_list))
    print(first_part_list)
    print("SPLIT")
    print(second_part_list)
    print("FINITO")
    output_sentence = "TESTESTESTESTESTEST"
    output_label = 199
    for i in range(len(first_part_list)):
        if ("die" in first_part_list[i].lower() and "dat" in second_part_list[i].lower()) or ("dat" in first_part_list[i].lower() and "die" in second_part_list[i].lower()):

            if label is 0:
                if first_part_list[i].lower().contains("die"):
                    first_part_list[i] = first_part_list[i].lower().replace('die','[MASK]')
                    output_sentence = " ".join(first_part_list).lower()
                    output_label = 0
                    if first_part_list[i].lower() != "die":
                        print("YOOOOOOOOOOOOOO")
                        print("")
                        print(first_part_list[i].lower())
                        print("")
                        print("YOOOOOOOOOOOOOO")
                else:
                    first_part_list[i] = first_part_list[i].lower().replace('dat','[MASK]')
                    output_sentence = " ".join(first_part_list).lower()
                    output_label = 1
                    if first_part_list[i].lower() != "dat":
                        print("YOOOOOOOOOOOOOO")
                        print("")
                        print(first_part_list[i].lower())
                        print("")
                ...
                if second_part_list[i].lower().contains("die"):
                    second_part_list[i] = second_part_list[i].lower().replace('die','[MASK]')
                    output_sentence = " ".join(second_part_list).lower()
                    output_label = 0
                    if second_part_list[i].lower() != "die":
                        print("YOOOOOOOOOOOOOO")
                        print("")
                        print(second_part_list[i].lower())
                        print("")
                        print("YOOOOOOOOOOOOOO")

                else:
                    second_part_list[i] = second_part_list[i].lower().replace('dat','[MASK]')
                    output_sentence = " ".join(second_part_list).lower() 
                    output_label = 1
                    if second_part_list[i].lower() != "dat":
                        print("YOOOOOOOOOOOOOO")
                        print("")
                        print(second_part_list[i].lower())
                        print("")
    ...
    diedat_train_sentences_masked.append(output_sentence)
    diedat_train_labels_masked.append(output_label)

print(diedat_train_sentences_masked[:5])
print(diedat_train_labels_masked[:5])


with open("TrainFiles-Die-Dat/diedat_train_sentences_masked_FROGMERGE", 'wb') as outputfile:
    pickle.dump(diedat_train_sentences_masked, outputfile)

with open("TrainFiles-Die-Dat/diedat_train_labels_masked_FROGMERGE", 'wb') as outputfile:
    pickle.dump(diedat_train_labels_masked, outputfile)
        

diedat_dev_sentences_masked = []
diedat_dev_labels_masked = []

for index in range(len(diedat_dev_sentences)):
    label_newline = diedat_dev_labels[index]
    sentence_newline = diedat_dev_sentences[index]
    sentence = sentence_newline.rstrip('\n')
    label = label_newline.rstrip('\n')
    two_parts = sentence.split("<sep>")
    first_part = two_parts[0]
    ...
    print(len(second_part_list))
    for i in range(len(first_part_list)):
        if (first_part_list[i].lower().contains("die") and second_part_list[i].lower().contains("dat")) or (first_part_list[i].lower().contains("dat") and second_part_list[i].lower().contains("die")):
            if label is 0:
                if first_part_list[i].lower().contains("die"):
                    first_part_list[i] = first_part_list[i].lower().replace('die','[MASK]')
                    output_sentence = " ".join(first_part_list).lower()
    ...
    diedat_dev_sentences_masked.append(output_sentence)
    diedat_dev_labels_masked.append(output_label)


with open("TrainFiles-Die-Dat/diedat_dev_sentences_masked_FROGMERGE", 'wb') as outputfile:
    pickle.dump(diedat_dev_sentences_masked, outputfile)

with open("TrainFiles-Die-Dat/diedat_dev_labels_masked_FROGMERGE", 'wb') as outputfile:
    pickle.dump(diedat_dev_labels_masked, outputfile)

print(diedat_dev_sentences_masked[:5])
print(diedat_dev_labels_masked[:5])
...
# #print("finished fit")
#
# #encoder.save("SavedTokenizers/FrogMorph1500k.json")
# #print("finished save")
# #
#
# #encoder = Encoder.load('SavedTokenizers/FrogMorph1000k128seq.json')
...
# # for sentence in FrogMorphedMergedEuroparlTotal:
# #     sentence_ids,_ = encoder.transform(sentence, fixed_length = fixed_len, frog_merge=True)
# #     if i % 1000 == 0:
...
# # pd.Series(sentences_ids_len).hist()
# # pd.Series(sentences_ids_len).describe().to_csv("InfoDatasetTINYBERT/oscar_1800k_to_2000k.csv")
# # print(pd.Series(sentences_ids_len).describe())
# # plt.xlabel = "Sentence Length"
# # plt.ylabel = "Amount of sentences"
# # #plt.legend()
# # plt.savefig("InfoDatasetTINYBERT/oscar_1800k_to_2000k.png",format='png')
#
...
# # Save features ###
#
# #with open('TrainFiles-BERT-TINY/europarl_sentences_ids.pickle', 'wb') as outputfile:
# # pickle.dump(europarl_sentences_ids, outputfile)
#
# with open('TrainFiles-BERT-TINY/oscar_sentences_ids_1500k_2000k_128seq.pickle', 'wb') as outputfile:
...
# # # sentences_ids_len = [len(x) for x in for_length_europarl_sentences_ids]
# # # pd.Series(sentences_ids_len).hist()
# # # pd.Series(sentences_ids_len).describe().to_csv("InfoDatasetTINYBERT/europarl-morfessor-1000k1000k.csv")
# # # print(pd.Series(sentences_ids_len).describe())
# # # plt.xlabel = "Sentence Length"
# # # plt.ylabel = "Amount of sentences"
# # # #plt.legend()
# # # plt.savefig("InfoDatasetTINYBERT/europarl-morfessor-1000k1000k.png",format='png')
# # #
# # #
# # #
#
