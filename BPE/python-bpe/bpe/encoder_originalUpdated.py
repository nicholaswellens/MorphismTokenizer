...
class Encoder:
    ...
    ###
    def transform_without_special_word_tokens(self, sentences, reverse=False, fixed_length=None):
        ID_encoding = next(self.transform(sentences, reverse))

        ###OPTIONAL: REMOVE EOW SOW ID's###
...
#        #Did here because other one still uses EOW/SOW so length is skewed
#        if fixed_length is not None:
#          ID_encoding = ID_encoding[:fixed_length]
#          while len(ID_encoding) < fixed_length:
#              ID_encoding.append(self.word_vocab[self.PAD])
